...
class BasicBlock(nn.Module):
    def __init__(self, in_channels, out_channels, groups=1):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=5, padding=2, groups=groups),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(),
            nn.Conv2d(out_channels, out_channels, kernel_size=5, padding=2, groups=groups),
            nn.BatchNorm2d(out_channels)
        )
...
class BasicUNet(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3):
        ...
        self.down_layers = torch.nn.ModuleList([
            ...
            BasicBlock(256, 512, groups=1),
        ])
        self.up_layers = torch.nn.ModuleList([
            BasicBlock(512, 256, groups=1),
            BasicBlock(512, 128, groups=1),
