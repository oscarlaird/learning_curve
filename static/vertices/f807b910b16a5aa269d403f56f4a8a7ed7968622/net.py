...
class BasicUNet(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3):
        ...
        self.down_layers = torch.nn.ModuleList([
            BasicBlock(in_channels + 1, 64, groups=1),   
            BasicBlock(64, 128, groups=1),
            BasicBlock(128, 256, groups=1),
        ])
        self.up_layers = torch.nn.ModuleList([
            BasicBlock(256, 128, groups=1),
            BasicBlock(128, 64, groups=1),
            BasicBlock(64, out_channels, groups=1),
        ])

        # don't use any activation function
        # since the BasicBlock already applies ReLU
        self.act = nn.Identity() # The activation function
